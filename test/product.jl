...
@testset "Testing generic VectorOfUnivariateDistribution" begin
    ...
    @test d_product1 isa Distributions.VectorOfUnivariateDistribution{<:Vector,ContinuousSupport,Float64}

    d_product2 = @inferred(product_distribution(ntuple(i -> Uniform(0.0, ubound[i]), 11)...))
    @test d_product2 isa Distributions.VectorOfUnivariateDistribution{<:Tuple,ContinuousSupport,Float64}
    ...
    @test d_product3 isa Distributions.VectorOfUnivariateDistribution{<:Fill,ContinuousSupport,Float64}
...
@testset "Testing discrete non-parametric VectorOfUnivariateDistribution" begin
    ...
    for a in ([0, 1], [-0.5, 0.5])
        ...
        # when `Product` is removed
        d_product1 = @inferred(Distributions.ProductDistribution(ds1))
        @test d_product1 isa Distributions.VectorOfUnivariateDistribution{<:Vector{<:DiscreteNonParametric},DiscreteSupport,eltype(a)}

        d_product2 = @inferred(product_distribution(ntuple(_ -> DiscreteNonParametric(a, [0.5, 0.5]), 11)...))
        @test d_product2 isa Distributions.VectorOfUnivariateDistribution{<:NTuple{N,<:DiscreteNonParametric},DiscreteSupport,eltype(a)}
        ...
        # when `Product` is removed
        d_product3 = @inferred(Distributions.ProductDistribution(ds3))
        @test d_product3 isa Distributions.VectorOfUnivariateDistribution{<:Fill{<:DiscreteNonParametric,1},DiscreteSupport,eltype(a)}
...
@testset "Testing tuple of continuous and discrete distribution" begin
    ...
    @test d_product isa Distributions.VectorOfUnivariateDistribution{<:Tuple,ContinuousSupport,Float64}
...
@testset "Testing generic MatrixOfUnivariateDistribution" begin
    ...
    ds1 = Uniform.(0.0, ubound)
    d_product1 = @inferred(product_distribution(ds1))
    @test d_product1 isa Distributions.MatrixOfUnivariateDistribution{<:Matrix{<:Uniform},ContinuousSupport,Float64}

    ds2 = Fill(Uniform(0.0, first(ubound)), M, N)
    d_product2 = @inferred(product_distribution(ds2))
    @test d_product2 isa Distributions.MatrixOfUnivariateDistribution{<:Fill{<:Uniform,2},ContinuousSupport,Float64}
...
@testset "Testing generic array of multivariate distribution" begin
    ...
    for N in ((11,), (11, 3))
        ...
        d_product1 = @inferred(product_distribution(ds1))
        @test d_product1 isa Distributions.ProductDistribution{length(N) + 1,1,<:Array{<:Dirichlet{Float64},length(N)},ContinuousSupport,Float64}

        ds2 = Fill(Dirichlet(first(alphas)), N...)
        d_product2 = @inferred(product_distribution(ds2))
        @test d_product2 isa Distributions.ProductDistribution{length(N) + 1,1,<:Fill{<:Dirichlet{Float64},length(N)},ContinuousSupport,Float64}
