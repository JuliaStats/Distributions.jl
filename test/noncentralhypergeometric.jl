...
@test mean(d) ≈ 71.95759 atol=1e-5
@test mode(d) == 72
@test quantile(d, 0.05) == 68
@test quantile(d, 0.95) == 75
@test logpdf(d, 75) ≈ -2.600957   atol=1e-6
@test pdf(d, 75)    ≈  0.07420252 atol=1e-7
@test cdf(d, 75)    ≈  0.9516117  atol=1e-7

ω = 0.361
d = FisherNoncentralHypergeometric(ns, nf, n, ω)

@test mean(d) ≈ 50.42299 atol=1e-5
@test mode(d) == 50
@test quantile(d, 0.05) == 47
@test quantile(d, 0.95) == 54
@test logpdf(d, 50) ≈ -1.820071  atol=1e-6
@test pdf(d, 50)    ≈  0.1620142 atol=1e-7
@test cdf(d, 50)    ≈  0.5203835 atol=1e-7
...
@test mean(d)           ≈ mean(ref)           atol=1e-5
@test logpdf(d, 51)     ≈ logpdf(ref, 51)     atol=1e-7
@test pdf(d, 51)        ≈ pdf(ref, 51)        atol=1e-7
@test cdf(d, 51)        ≈ cdf(ref, 51)        atol=1e-7
@test quantile(d, 0.05) ≈ quantile(ref, 0.05) atol=1e-7
@test quantile(d, 0.95) ≈ quantile(ref, 0.95) atol=1e-7
...
@test mean(d)       ≈ 78.82945    atol=1e-5
@test mode(d)           == 80
@test quantile(d, 0.05) == 77
@test quantile(d, 0.95) == 80
@test logpdf(d, 75) ≈ -4.750073   atol=1e-6
@test pdf(d, 75)    ≈  0.00865106 atol=1e-7
@test cdf(d, 75)    ≈  0.01133378 atol=1e-7

ω = 0.361
d = WalleniusNoncentralHypergeometric(ns, nf, n, ω)

@test mean(d)       ≈ 45.75323  atol=1e-5
@test mode(d)           == 45
@test quantile(d, 0.05) == 42
@test quantile(d, 0.95) == 49
@test logpdf(d, 50) ≈ -3.624478 atol=1e-6
@test pdf(d, 50)    ≈  0.026663 atol=1e-7
@test cdf(d, 50)    ≈  0.983674 atol=1e-7
