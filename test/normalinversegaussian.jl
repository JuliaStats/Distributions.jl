...
@test params(d)[1] ≈ 1.7 atol=0.000000001
@test params(d)[2] ≈ 1.8 atol=0.000000001
@test params(d)[3] ≈ 1.2 atol=0.000000001
@test params(d)[4] ≈ 2.3 atol=0.000000001

# The solution was computed using this R code:
# dnig(4.2, mu=1.7, alpha=1.8, beta=1.2, delta=2.3)
@test pdf(d, 4.2) ≈ 0.2021958 atol=0.0000001

# The solution was computed using this R code:
# dnig(4.8, mu=1.7, alpha=1.8, beta=1.2, delta=2.3, log=TRUE)
@test logpdf(d, 4.8) ≈ -1.909973 atol=0.000001

# The solution was computed using this R code:
# mean(rnig(100000000, mu=1.7, alpha=1.8, beta=1.2, delta=2.3))
@test mean(d) ≈ 3.757509 atol=0.001

# The solution was computed using this R code:
# var(rnig(100000000, mu=1.7, alpha=1.8, beta=1.2, delta=2.3))
@test var(d) ≈ 3.085488 atol=0.001

# The solution was computed using this R code:
# skewness(rnig(100000000, mu=1.7, alpha=1.8, beta=1.2, delta=2.3))
@test skewness(d) ≈ 1.138959 atol=0.001
