...
@testset "LogLogistic" begin
	@test isa(covert(LogLogistic{Float64}, Float16(0), Float16(1)), LogLogistic{Float64}) 
	@test mean(LogLogistic(sin(1), pi)) === 1.0 
	@test_throws mean(LogLogistic(1, 1))
	@test mode(LogLogistic(sqrt(3), 2)) === 1.0
	@test_throws mean(LogLogistic(1, 1)) 
	@test var(LogLogistic(1, pi)) ≈ 0.7872174131518408 rtol = 1e-12  
	@test_throws var(LogLogistic(1, 1)) 
	@test Test.detect_ambiguities(LogLogistic)
