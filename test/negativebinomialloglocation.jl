...
@testset "Check the corner case p==1" begin
    d = convert(NegativeBinomialLogLocation{Float64}, NegativeBinomial(0.5, 1.0))
    @test logpdf(d, 0) === 0.0
    @test logpdf(d, 1) === -Inf
end

@testset "Interconversion checks" begin
    d = NegativeBinomialLogLocation()
    nb = NegativeBinomial()
    ...
    for (d₂, T) ∈ zip((nb, nbl, nbll, nbpg), (NegativeBinomial, NegativeBinomialLocation, NegativeBinomialLogLocation, NegativeBinomialPoissonGamma))
        # Convert to other distribution from NegativeBinomialLogLocation
        @test d₂ == convert(T{Float64}, d)
        # Convert from other distribution to NegativeBinomialLogLocation
        @test d == convert(NegativeBinomialLogLocation{Float64}, d₂)
    end
end
...
for (params, xs) ∈ zip(((), (6,), (1, 0.5), (5, 0.6), (0.5, 0.5)),
    ...
    @testset "Checks against NegativeBinomial($(join(params, ',')))" begin
        nb = NegativeBinomial()
        d = convert(NegativeBinomialLogLocation{Float64}, nb)
        @test minimum(d) == 0
        @test maximum(d) == Inf
        @test insupport(d, -0)
