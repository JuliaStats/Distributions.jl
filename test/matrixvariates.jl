...
@testset "matrixvariates" begin
    ...
    function test_convert(d::MatrixDistribution)
        ...
        for elty in (Float32, Float64, BigFloat)
            ...
            del2 =
                convert(distname{elty}, (Base.Fix1(getfield, d)).(fieldnames(typeof(d)))...)
    ...
    test_against_multivariate(dist::Type{<:MatrixDistribution}, n::Integer, p::Integer) =
        nothing
    ...
    function test_special(dist::Type{MatrixNormal})
        ...
        @testset "Non-allocating sampling" begin
            ...
            noallocD =
                MatrixNormal(M, cholesky!(Symmetric(U, :L)), cholesky!(Symmetric(V, :U)))
