...
# their entries recursively, a distribution contributes its own `partype`, and anything not
# real-valued (`nothing`, or the `uplo` of `LKJCholesky`) contributes nothing.
...
recursive_partype(x) = eltype(x) <: Real ? eltype(x) : Union{}
...
function test_partype(d::Distribution)
    ...
    @test @inferred(partype(d)) === @inferred(partype(typeof(d)))
    @test @inferred(partype(d)) ===
