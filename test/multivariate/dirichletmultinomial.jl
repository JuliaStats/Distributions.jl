...
@testset "Testing DirichletMultinomial with $key" for (key, func) in Dict(
    ...
    @test isapprox(mean(d), vec(mean(x, dims = 2)), atol = 0.5)
    @test isapprox(std(d), vec(std(x, dims = 2)), atol = 0.5)
    @test isapprox(var(d), vec(var(x, dims = 2)), atol = 0.5)
    @test isapprox(cov(d), cov(x, dims = 2), atol = 0.5)

    # test Evaluation
    @testset "Dirichlet evaluation and non-Vector arguments" begin
        αS = SVector(0.1, 0.7, 9.2, 3.7, 0.4)
        for (d, T) in (
            DirichletMultinomial(10, 5) => DirichletMultinomial{Float64,Vector{Float64}},
            ...
            for x in
                (2 * ones(5), [1, 2, 3, 4, 0], [3.0, 0.0, 3.0, 0.0, 4.0], [0, 0, 0, 0, 10])
                @test pdf(d, x) ≈
    ...
    @test isapprox(mle.α, d.α, atol = 0.2)
    mle = fit(DirichletMultinomial{Float64}, x)
    @test isapprox(mle.α, d.α, atol = 0.2)

    # test MLE with weights
    for w in (0.1 * ones(10_000), ones(10_000), 10 * ones(10_000))
