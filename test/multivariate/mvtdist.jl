...
@testset "mvtdist" begin
    ...
    @test size(rand(MvTDist(1.0, mu, Sigma))) == (2,)
    @test size(rand(MvTDist(1.0, mu, Sigma), 10)) == (2, 10)
    @test size(rand(MersenneTwister(123), MvTDist(1.0, mu, Sigma))) == (2,)
    @test size(rand(MersenneTwister(123), MvTDist(1.0, mu, Sigma), 10)) == (2, 10)

    # static array for mean/variance
    mu_static = @SVector [1.0, 2]
    ...
    for i = 1:length(df)
        ...
        @test isapprox(logpdf(d, [-2.0, 3]), rvalues[i], atol = 1.0e-8)
        @test isa(logpdf(d32, [-2.0f0, 3.0f0]), Float32)
        @test isapprox(
            logpdf(d32, [-2.0f0, 3.0f0]),
            convert(Float32, rvalues[i]),
            atol = 1.0e-4,
        )
        dd = typeof(d)(params(d)...)
        @test d.df == dd.df
