...
logvmfCp(p::Int, κ::Real) =
    (p / 2 - 1) * log(κ) - log(besselix(p / 2 - 1, κ)) - κ - p / 2 * log(2π)
...
function test_angle3(κ::Real, ns::Int, rng::Union{AbstractRNG,Missing} = missing)
    ...
    angle3_res = [Distributions._vmf_angle3(rng, spl.κ) for _ = 1:ns]
    angle_res = [Distributions._vmf_angle(rng, spl) for _ = 1:ns]
