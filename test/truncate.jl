...
module TestTruncate
        ...
        for pt in pts
            ...
            if !isa(
                    ...
                    Distributions.Truncated{
                        Distributions.StudentizedRange{Float64},
            ...
            if !any(
                    ...
                    [
                        ...
                        Distributions.Truncated{
                            Distributions.NoncentralF{Float64},
                            Distributions.Continuous,
                            Float64,
                        },
                        Distributions.Truncated{
                            Distributions.NoncentralT{Float64},
                            Distributions.Continuous,
                            Float64,
                        },
                        Distributions.Truncated{
    ...
    at = [0.0, 1.0, 0.0, 1.0]
    @test isapprox(ForwardDiff.gradient(f, at), fdm(f, at), atol = 1.0e-6)
    ...
    @testset "#1328" begin
        ...
        @test logsumexp(logpdf(dist, x) for x in 0:1000) ≈ 0 atol = 1.0e-15
        @test logsumexp(logpdf(dist_zeroinflated, x) for x in 0:1000) ≈ 0 atol = 1.0e-15
        @test logsumexp(logpdf(dist_zerotruncated, x) for x in 0:1000) ≈ 0 atol = 1.0e-15
        @test logsumexp(logpdf(dist_zeromodified, x) for x in 0:1000) ≈ 0 atol = 1.0e-15
    end
end

@testset "show" begin
    @test sprint(show, "text/plain", truncated(Normal(); lower = 2.0)) ==
        "Truncated($(Normal()); lower=2.0)"
    @test sprint(show, "text/plain", truncated(Normal(); upper = 3.0)) ==
        "Truncated($(Normal()); upper=3.0)"
    @test sprint(show, "text/plain", truncated(Normal(), 2.0, 3.0)) ==
...
@testset "quantile examples (#1726)" begin
    ...
    @test isa(quantile(d_narrow, ForwardDiff.Dual(0.0, 0.0)), ForwardDiff.Dual)
    ...
    @test isa(quantile(d, ForwardDiff.Dual(1.0, 0.0)), ForwardDiff.Dual)
end
