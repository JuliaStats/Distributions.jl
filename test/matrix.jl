...
@test mean(rand(W,100000))  ≈ mean(W)  atol=0.1
@test mean(rand(IW,100000)) ≈ mean(IW) atol=0.1

v = 3.0

@test pdf(Wishart(v,S), S)          ≈ 0.04507168 atol=1e-8
@test pdf(Wishart(v,S), inv(S))     ≈ 0.01327698 atol=1e-8
@test pdf(Wishart(v,inv(S)),S)      ≈ 0.0148086  atol=1e-8
@test pdf(Wishart(v,inv(S)),inv(S)) ≈ 0.01901462 atol=1e-8
...
@test pdf(InverseWishart(v,S), S)          ≈ 0.04507168  atol=1e-8
@test pdf(InverseWishart(v,S), inv(S))     ≈ 0.006247377 atol=1e-8
@test pdf(InverseWishart(v,inv(S)),S)      ≈ 0.03147137  atol=1e-8
@test pdf(InverseWishart(v,inv(S)),inv(S)) ≈ 0.01901462  atol=1e-8
