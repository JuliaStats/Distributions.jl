...
@testset "namedparams" begin
    dists = Any[
        ...
        truncated(JohnsonSU(0.0, 1.0, 0.0, 1.0); lower=0.0),
        OrderStatistic(JohnsonSU(0.0, 1.0, 0.0, 1.0), 5, 2),
    ...
    @testset "all distributions are covered" begin
        # `Distributions` itself or one of its submodules
        indistributions(m::Module) =
            m === Distributions || (parentmodule(m) !== m && indistributions(parentmodule(m)))
        ...
        @testset "$T" for T in types
            ...
            indistributions(parentmodule(T)) || continue
    ...
    @testset "reconstruction" begin
        ...
        @testset "$f" for (f, d) in [
            ...
            MvNormalCanon => MvNormalCanon(ones(2), PDMat(Matrix(1.0I, 2, 2))),
