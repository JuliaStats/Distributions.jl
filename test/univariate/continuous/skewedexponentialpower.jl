...
@testset "SkewedExponentialPower" begin
    @testset "α = 0.5" begin
        ...
        @test @inferred params(d2) == (0.0, 1.0, 1.0, 0.5)
    ...
    @testset "α != 0.5" begin
        ...
        test = [
            -10.0 0.004770878 0.02119061
            -9.2631579 0.005831228 0.0250811
            ...
            4.0 0.005718906 0.98942757
        ]

        d = SkewedExponentialPower(0, 1, 0.5, 0.7)
        for t in eachrow(test)
            @test @inferred(pdf(d, t[1])) ≈ t[2] rtol = 1.0e-5
            @test @inferred(logpdf(d, t[1])) ≈ log(t[2]) rtol = 1.0e-5
            @test @inferred(cdf(d, t[1])) ≈ t[3] rtol = 1.0e-3
            @test @inferred(logcdf(d, t[1])) ≈ log(t[3]) rtol = 1.0e-3
        end
        test_distr(d, 10^6)
        ...
        moments = [
            (2 * p^(1 / p))^k *
                ((-1)^k * α^(1 + k) + (1 - α)^(1 + k)) *
                gamma((1 + k) / p) / gamma(1 / p) for k in 1:4
        ]
