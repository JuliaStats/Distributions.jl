...
@testset "LogUniform" begin
    ...
    @test pdf(LogUniform(1.0f0, 2.0f0), 1) isa Float32
    @test pdf(LogUniform(1, 2), 1.0f0) isa Float32
    @test pdf(LogUniform(1, 2), 1) isa Float64
    @test quantile(LogUniform(1, 2), 1) isa Float64
    @test quantile(LogUniform(1, 2), 1.0f0) isa Float32
    @testset "$f" for f in [pdf, cdf, quantile, logpdf, logcdf]
        ...
        @test @inferred(f(LogUniform(1.0f0, 2), 1.0f0)) isa Float32
        @test @inferred(f(LogUniform(1, 2), 1.0f0)) isa Float32
    end
