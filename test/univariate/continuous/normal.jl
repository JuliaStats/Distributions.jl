...
@testset "Normal" begin
    ...
    logdiffcdf_big(d::Normal, x::Real, y::Real) =
        logsubexp(logcdf(d, big(y)), logcdf(d, big(x)))
...
@testset "Normal logpdf & pdf type inference" begin
    ...
    @test @inferred(quantile(Normal(1.0f0, 0.0f0), 1 // 2)) === 1.0f0
    @test @inferred(quantile(Normal(1.0f0, 0.0), 1 // 2)) === 1.0
    ...
    @test @inferred(cquantile(Normal(1.0f0, 0.0f0), 1 // 2)) === 1.0f0
    @test @inferred(cquantile(Normal(1.0f0, 0.0), 1 // 2)) === 1.0
end
