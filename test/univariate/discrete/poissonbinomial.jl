...
@testset "poissonbinomial" begin
    ...
    x = [3.5118, 0.6219, 0.2905, 0.845, 1.8648]
    n = length(x)
    p = x ./ (1 .+ x)
    ...
    for (p, n) in [(0.8, 6), (0.5, 10), (0.04, 20)]
        ...
        for i in 0:n
            @test @inferred(pdf(d, i)) ≈ pdf(dref, i) atol = 1.0e-14
            @test @inferred(pdf(d, i // 1)) ≈ pdf(dref, i) atol = 1.0e-14
            @test @inferred(logpdf(d, i)) ≈ logpdf(dref, i)
            @test @inferred(logpdf(d, i // 1)) ≈ logpdf(dref, i)
            for f in (cdf, ccdf, logcdf, logccdf)
                @test @inferred(f(d, i)) ≈ f(dref, i) rtol = 1.0e-6
                @test @inferred(f(d, i // 1)) ≈ f(dref, i) rtol = 1.0e-6
                @test @inferred(f(d, i + 0.5)) ≈ f(dref, i) rtol = 1.0e-6
            end
        end
    ...
    for (n₁, n₂, n₃, p₁, p₂, p₃) in [
        ...
        for k in 0:n
            ...
            for i in 0:min(n₁, k)
                mc = 0.0
                for j in i:min(i + n₂, k)
                    mc += (k - j <= n₃) && pmf2[j - i + 1] * pmf3[k - j + 1]
                end
                m += pmf1[i + 1] * mc
            end
            @test @inferred(pdf(d, k)) ≈ m atol = 1.0e-14
            @test @inferred(pdf(d, k // 1)) ≈ m atol = 1.0e-14
