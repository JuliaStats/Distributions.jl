...
@testset "Testing matrix-variates with $key" for (key, func) in Dict(
    ...
    # Invalid probability
    @test_throws DomainError DiscreteNonParametric(
        [40.0, 80, 120, -60],
        [0.5, 0.3, 0.1, 0.2],
    )

    # Invalid probability, but no arg check
    DiscreteNonParametric([40.0, 80, 120, -60], [0.5, 0.3, 0.1, 0.2], check_args = false)
    ...
    @test func[1](d) ∈ [40.0, 80.0, 120.0, -60.0]
    @test func[1](sampler(d)) ∈ [40.0, 80.0, 120.0, -60.0]
    ...
    @test pdf(d, -60) == 0.2
    @test pdf(d, 100.0) == 0.0
    @test pdf(d, 120.0) == 0.1

    @test iszero(cdf(d, -Inf))
    @test cdf(d, -100.0) == 0.0
    @test cdf(d, -100) == 0.0
    @test cdf(d, 0.0) ≈ 0.2
    @test cdf(d, 100.0) ≈ 0.9
    @test cdf(d, 100) ≈ 0.9
    @test cdf(d, 150.0) == 1.0
    @test isone(cdf(d, Inf))
    @test isnan(cdf(d, NaN))

    @test isone(ccdf(d, -Inf))
    @test ccdf(d, -100.0) == 1.0
    @test ccdf(d, -100) == 1.0
    @test ccdf(d, 0.0) ≈ 0.8
    @test ccdf(d, 100.0) ≈ 0.1
    @test ccdf(d, 100) ≈ 0.1
    @test ccdf(d, 150.0) == 0
    ...
    @test logcdf(d, -100.0) == -Inf
    @test logcdf(d, -100) == -Inf
    @test logcdf(d, 0.0) ≈ log(0.2)
    @test logcdf(d, 100.0) ≈ log(0.9)
    @test logcdf(d, 100) ≈ log(0.9)
    @test iszero(logcdf(d, 150.0))
    @test iszero(logcdf(d, Inf))
    @test isnan(logcdf(d, NaN))

    @test iszero(logccdf(d, -Inf))
    @test iszero(logccdf(d, -100.0))
    @test iszero(logccdf(d, -100))
    @test logccdf(d, 0.0) ≈ log(0.8)
    @test logccdf(d, 100.0) ≈ log(0.1)
    @test logccdf(d, 100) ≈ log(0.1)
    @test logccdf(d, 150.0) == -Inf
    @test logccdf(d, Inf) == -Inf
    @test isnan(logccdf(d, NaN))

    @test quantile(d, 0) == -60
    @test quantile(d, 0.1) == -60
    @test quantile(d, 0.3) == 40
    @test quantile(d, 0.7) == 80
    @test quantile(d, 1) == 120
    @test minimum(d) == -60
    @test maximum(d) == 120

    @test insupport(d, -60)
    @test insupport(d, -60.0)
    @test !insupport(d, 20)
    @test !insupport(d, 20.0)
    @test insupport(d, 80)
