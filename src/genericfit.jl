...
fit_mle(dt::Type{D}, x::AbstractArray) where {D<:UnivariateDistribution} =
    fit_mle(D, suffstats(D, x))
fit_mle(dt::Type{D}, x::AbstractArray, w::AbstractArray) where {D<:UnivariateDistribution} =
    fit_mle(D, suffstats(D, x, w))
...
fit_mle(
    ...
    x::AbstractMatrix,
    w::AbstractArray,
) where {D<:MultivariateDistribution} = fit_mle(D, suffstats(D, x, w))
