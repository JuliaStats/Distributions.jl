...
function Pareto(α::T, θ::T; arg_check = true) where {T <: Real}
    check_arg && @check_args(Pareto, α > zero(α) && θ > zero(θ))
    return Pareto{T}(α, θ)
end
...
Pareto() = Pareto(1.0, 1.0, arg_check = false)

@distr_support Pareto d.θ Inf

#### Conversions
convert(::Type{Pareto{T}}, α::Real, θ::Real) where {T<:Real} = Pareto(T(α), T(θ))
convert(::Type{Pareto{T}}, d::Pareto{S}) where {T <: Real, S <: Real} = Pareto(T(d.α), T(d.θ), arg_check = false)
