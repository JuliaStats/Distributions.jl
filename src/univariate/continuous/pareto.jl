...
function Pareto(α::T, θ::T; check_arg=true) where {T <: Real}
...
Pareto() = Pareto(1.0, 1.0, check_arg = false)

@distr_support Pareto d.θ Inf

#### Conversions
convert(::Type{Pareto{T}}, α::Real, θ::Real) where {T<:Real} = Pareto(T(α), T(θ))
convert(::Type{Pareto{T}}, d::Pareto{S}) where {T <: Real, S <: Real} = Pareto(T(d.α), T(d.θ), check_arg = false)
