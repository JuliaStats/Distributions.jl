...
function BetaPrime(α::T, β::T; check_arg=true) where {T<:Real}
    check_arg && @check_args(BetaPrime, α > zero(α) && β > zero(β))
...
BetaPrime() = BetaPrime(1.0, 1.0, check_arg = false)
...
function convert(::Type{BetaPrime{T}}, d::BetaPrime{S}) where {T <: Real, S <: Real}
    BetaPrime(T(d.α), T(d.β), check_arg = false)
end
