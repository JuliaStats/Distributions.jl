...
function VonMises(μ::T, κ::T; check_arg=true) where {T <: Real}
    check_arg && @check_args(VonMises, κ > zero(κ))
...
VonMises() = VonMises(0.0, 1.0, check_arg = false)
...
convert(::Type{VonMises{T}}, d::VonMises{S}) where {T<:Real, S<:Real} = VonMises(T(d.μ), T(d.κ), check_arg = false)
