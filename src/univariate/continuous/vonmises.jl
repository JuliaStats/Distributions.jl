...
function VonMises(μ::T, κ::T; arg_check=true) where {T <: Real}
    arg_check && @check_args(VonMises, κ > zero(κ))
...
VonMises() = VonMises(0.0, 1.0, arg_check = false)
...
convert(::Type{VonMises{T}}, d::VonMises{S}) where {T<:Real, S<:Real} = VonMises(T(d.μ), T(d.κ), arg_check = false)
