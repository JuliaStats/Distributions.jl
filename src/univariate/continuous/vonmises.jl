...
function VonMises(μ::T, κ::T; check_args=true) where {T <: Real}
    ...
    return VonMises{T}(μ, κ, SpecialFunctions.besselix(zero(T), κ))
end
...
mode(d::VonMises) = d.μ
var(d::VonMises) = 1 - SpecialFunctions.besselix(1, d.κ) / d.I0κx
# deprecated 12 September 2016
@deprecate circvar(d) var(d)
entropy(d::VonMises) = log(twoπ * d.I0κx) + d.κ * (1 - SpecialFunctions.besselix(1, d.κ) / d.I0κx)

cf(d::VonMises, t::Real) = (SpecialFunctions.besselix(abs(t), d.κ) / d.I0κx) * cis(t * d.μ)
...
pdf(d::VonMises{T}, x::Real) where T<:Real =
    -π ≤ x ≤ π ? exp(d.κ * (cos(x - d.μ) - 1)) / (twoπ * d.I0κx) : zero(T)
logpdf(d::VonMises{T}, x::Real) where T<:Real =
    -π ≤ x ≤ π ? d.κ * (cos(x - d.μ) - 1) - log(d.I0κx) - log2π : -T(Inf)
...
function _vmcdf(κ::Real, I0κx::Real, x::Real, tol::Real)
    tol *= exp(-κ)
    j = 1
    cj = SpecialFunctions.besselix(j, κ) / j
    s = cj * sin(j * x)
    while abs(cj) > tol
        j += 1
        cj = SpecialFunctions.besselix(j, κ) / j
