...
function TDist(ν::T; check_arg=true) where {T <: Real}
    check_arg && @check_args(TDist, ν > zero(ν))
    return TDist{T}(ν)
end
...
convert(::Type{TDist{T}}, d::TDist{S}) where {T<:Real, S<:Real} = TDist(T(d.ν), check_arg = false)
