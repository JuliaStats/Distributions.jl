...
Epanechnikov(μ::Real, σ::Real; check_args::Bool = true) =
    Epanechnikov(promote(μ, σ)...; check_args = check_args)
Epanechnikov(μ::Integer, σ::Integer; check_args::Bool = true) =
    Epanechnikov(float(μ), float(σ); check_args = check_args)
...
function convert(::Type{Epanechnikov{T}}, μ::Real, σ::Real) where {T <: Real}
    return Epanechnikov(T(μ), T(σ), check_args = false)
end
function Base.convert(::Type{Epanechnikov{T}}, d::Epanechnikov) where {T <: Real}
    return Epanechnikov{T}(T(d.μ), T(d.σ))
end
...
function mgf(d::Epanechnikov{T}, t::Real) where {T <: Real}
    a = d.σ * t
    return a == 0 ? one(T) : 3exp(d.μ * t) * (cosh(a) - sinh(a) / a) / a^2
end

function cf(d::Epanechnikov{T}, t::Real) where {T <: Real}
    a = d.σ * t
    return a == 0 ? one(T) + zero(T) * im : -3exp(im * d.μ * t) * (cos(a) - sin(a) / a) / a^2
end
