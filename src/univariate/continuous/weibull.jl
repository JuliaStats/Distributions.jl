...
Weibull(α::Real, θ::Real; check_args::Bool = true) =
    Weibull(promote(α, θ)...; check_args = check_args)
Weibull(α::Integer, θ::Integer; check_args::Bool = true) =
    Weibull(float(α), float(θ); check_args = check_args)
