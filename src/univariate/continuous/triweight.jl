...
Triweight(μ::Real, σ::Real; check_args::Bool = true) =
    Triweight(promote(μ, σ)...; check_args = check_args)
Triweight(μ::Integer, σ::Integer; check_args::Bool = true) =
    Triweight(float(μ), float(σ); check_args = check_args)
...
Base.convert(::Type{Triweight{T}}, d::Triweight) where {T <: Real} =
    Triweight{T}(T(d.μ), T(d.σ))
...
function pdf(d::Triweight{T}, x::Real) where {T <: Real}
    u = abs(x - d.μ) / d.σ
    return u >= 1 ? zero(T) : 1.09375 * (1 - u * u)^3 / d.σ
end
logpdf(d::Triweight, x::Real) = log(pdf(d, x))

function cdf(d::Triweight{T}, x::Real) where {T <: Real}
    u = (x - d.μ) / d.σ
    return u ≤ -1 ? zero(T) : u ≥ 1 ? one(T) : 0.03125 * (1 + u)^4 * @horner(u, 16, -29, 20, -5)
end
...
function mgf(d::Triweight{T}, t::Float64) where {T <: Real}
    ...
    a2 = a * a
    return a == 0 ? one(T) :
        105 * exp(d.μ * t) * ((15 / a2 + 1) * cosh(a) - (15 / a2 - 6) / a * sinh(a)) / (a2 * a2)
end

function cf(d::Triweight{T}, t::Float64) where {T <: Real}
    a = d.σ * t
    a2 = a * a
    return a == 0 ? complex(one(T)) :
        105 * cis(d.μ * t) * ((1 - 15 / a2) * cos(a) + (15 / a2 - 6) / a * sin(a)) / (a2 * a2)
end
