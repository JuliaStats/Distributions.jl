...
NoncentralT(ν::Real, λ::Real; check_args::Bool = true) =
    NoncentralT(promote(ν, λ)...; check_args = check_args)
NoncentralT(ν::Integer, λ::Integer; check_args::Bool = true) =
    NoncentralT(float(ν), float(λ); check_args = check_args)
