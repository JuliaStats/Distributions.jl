...
TriangularDist(a::Real, b::Real, c::Real; check_args::Bool = true) =
    TriangularDist(promote(a, b, c)...; check_args = check_args)
function TriangularDist(a::Integer, b::Integer, c::Integer; check_args::Bool = true)
    return TriangularDist(float(a), float(b), float(c); check_args = check_args)
end
...
#### Conversions
convert(::Type{TriangularDist{T}}, a::Real, b::Real, c::Real) where {T <: Real} =
    TriangularDist(T(a), T(b), T(c))
Base.convert(::Type{TriangularDist{T}}, d::TriangularDist) where {T <: Real} =
    TriangularDist{T}(T(d.a), T(d.b), T(d.c))
...
function median(d::TriangularDist)
    (a, b, c) = params(d)
    m = middle(a, b)
    return c >= m ? a + sqrt((b - a) * (c - a) / 2) : b - sqrt((b - a) * (b - c) / 2)
end
...
function var(d::TriangularDist)
    (a, b, c) = params(d)
    return _pretvar(a, b, c) / 18
end

function skewness(d::TriangularDist{T}) where {T <: Real}
    (a, b, c) = params(d)
    return sqrt2 * (a + b - 2c) * (2a - b - c) * (a - 2b + c) / (5 * _pretvar(a, b, c)^(T(3) / 2))
end
...
function quantile(d::TriangularDist, p::Real)
    ...
    b_m_a = b - a
    rl = c_m_a / b_m_a
    return p <= rl ? a + sqrt(b_m_a * c_m_a * p) : b - sqrt(b_m_a * (b - c) * (1 - p))
end
...
function mgf(d::TriangularDist, t::Real)
    ...
    if a < c
        if c < b
            # Case: a < c < b
            return exp(c * t) *
                ((c - a) * _phi2((a - c) * t) + (b - c) * _phi2((b - c) * t)) / (b - a)
        else
            # Case: a < c = b
...
function cf(d::TriangularDist, t::Real)
    ...
    if a < c
        if c < b
            # Case: a < c < b
            return cis(c * t) *
                ((c - a) * _cisphi2((a - c) * t) + (b - c) * _cisphi2((b - c) * t)) /
                (b - a)
        else
            # Case: a < c = b
...
function rand(rng::AbstractRNG, d::TriangularDist)
    ...
    b_m_a = b - a
    u = rand(rng)
    return b_m_a * u < (c - a) ? d.a + sqrt(u * b_m_a * (c - a)) :
