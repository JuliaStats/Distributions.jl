...
function Levy(μ::T, σ::T; check_arg=true) where {T}
    check_arg && @check_args(Levy, σ > zero(σ))
    return Levy{T}(μ, σ)
end
...
Levy() = Levy(0.0, 1.0, check_arg = false)
...
convert(::Type{Levy{T}}, d::Levy{S}) where {T <: Real, S <: Real} = Levy(T(d.μ), T(d.σ), check_arg = false)
