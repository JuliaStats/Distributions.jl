...
Laplace(μ::Real, θ::Real; check_args::Bool = true) =
    Laplace(promote(μ, θ)...; check_args = check_args)
Laplace(μ::Integer, θ::Integer; check_args::Bool = true) =
    Laplace(float(μ), float(θ); check_args = check_args)
...
invlogcdf(d::Laplace, lp::Real) =
    lp < loghalf ? xval(d, logtwo + lp) : xval(d, -(logtwo + log1mexp(lp)))
invlogccdf(d::Laplace, lp::Real) =
    lp > loghalf ? xval(d, logtwo + log1mexp(lp)) : xval(d, -(logtwo + lp))
