...
Biweight(μ::Real, σ::Real; check_args::Bool = true) =
    Biweight(promote(μ, σ)...; check_args = check_args)
Biweight(μ::Integer, σ::Integer; check_args::Bool = true) =
    Biweight(float(μ), float(σ); check_args = check_args)
...
function pdf(d::Biweight{T}, x::Real) where {T <: Real}
    u = abs(x - d.μ) / d.σ
    return u >= 1 ? zero(T) : (15 // 16) * (1 - u^2)^2 / d.σ
end
logpdf(d::Biweight, x::Real) = log(pdf(d, x))

function cdf(d::Biweight{T}, x::Real) where {T <: Real}
    u = (x - d.μ) / d.σ
    return u ≤ -1 ? zero(T) : u ≥ 1 ? one(T) : (u + 1)^3 / 16 * @horner(u, 8, -9, 3)
end
...
function mgf(d::Biweight{T}, t::Real) where {T <: Real}
    a = d.σ * t
    a2 = a^2
    return a == 0 ? one(T) : 15exp(d.μ * t) * (-3cosh(a) + (a + 3 / a) * sinh(a)) / (a2^2)
end

function cf(d::Biweight{T}, t::Real) where {T <: Real}
    a = d.σ * t
    a2 = a^2
    return a == 0 ? one(T) + zero(T) * im :
