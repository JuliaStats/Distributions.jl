...
let
    ...
    global function quantile(d::Chernoff, tau::Real)
        ...
        dnorm=Distributions.Normal(0.0,1.0)
        if tau<0.001 return -newton(tau-x->Fbar(x),f,quantile(dnorm,1.0-tau)*0.52) end
        if tau>0.999 return newton(x->1.0-tau-Fbar(x),f,quantile(dnorm,tau)*0.52) end
        return newton(x->F(x)-tau,f,quantile(dnorm,tau)*0.52)   # should consider replacing x-> construct for speed
    end

    global minimum(d::Chernoff)=-Inf
    global maximum(d::Chernoff)=Inf
    global insupport(d::Chernoff, x::Real)=isnan(x) ? false : true

    ### Recommended functions

    global mean(d::Chernoff) = 0.0
    global var(d::Chernoff) =0.26355964132470455
    global modes(d::Chernoff)=[0.0]
    global mode(d::Chernoff) = 0.0
    global skewness(d::Chernoff) = 0.0
    global kurtosis(d::Chernoff) = -0.16172525511461888
    global kurtosis(d::Chernoff,excess::Bool) = kurtosis(d) + (excess ? 0.0 : 3.0) 
    global entropy(d::Chernoff)=-0.7515605300273104

    ### Random number generation
    global slowrand(d::Chernoff)=quantile(d,rand(GLOBAL_RNG))
    ...
    global function rand(rng::AbstractRNG, d::Chernoff)                 # Ziggurat random number generator --- slow in the tails
        ...
        rabs=abs(r)
        if rabs<x[i+1] return r end
        s= (i>0) ? (y[i]+rand(rng)*(y[i+1]-y[i])) : rand(rng)*y[1]
        if s<2.0*f(rabs) return r end
        if i>0 return rand(rng,d) end
        F0=F(A/y[1])
        tau=2.0*rand(rng)-1
        tauabs=abs(tau)
