...
function PGeneralizedGaussian(μ::T1,α::T2,p::T3; arg_check = true) where {T1<:Real, T2<:Real, T3<:Real}
    arg_check && @check_args(PGeneralizedGaussian, α > zero(α) && p > zero(p))
...
"""
PGeneralizedGaussian() = PGeneralizedGaussian(0.0, √2, 2.0, arg_check = false) # approximate scale with unity std deviation and shape 2
...
function convert(::Type{PGeneralizedGaussian{T1,T2,T3}}, d::PGeneralizedGaussian{S1,S2,S3}) where {T1 <: Real, T2 <: Real, T3 <: Real, S1 <: Real, S2 <: Real, S3 <: Real}
    return PGeneralizedGaussian(T1(d.μ), T2(d.α), T3(d.p), arg_check = false)
end
