...
var(d::PGeneralizedGaussian) = (d.α^2) * (gamma(3 * inv(d.p)) / gamma(inv(d.p)))
std(d::PGeneralizedGaussian) = (d.α) * sqrt(gamma(3 * inv(d.p)) / gamma(inv(d.p)))

skewness(d::PGeneralizedGaussian{T1, T2, T3}) where {T1,T2,T3} = zero(T1)
kurtosis(d::PGeneralizedGaussian) = gamma(5 * inv(d.p)) * gamma(inv(d.p)) / (gamma(3 * inv(d.p))^2) - 3
entropy(d::PGeneralizedGaussian) = inv(d.p) - log( d.p / (2 * d.α * gamma(inv(d.p))))
...
function pdf(d::PGeneralizedGaussian, x::Real)
    (μ, α, p) = params(d)
    return ( p / ( 2α * gamma(1 / p) ) ) * exp( -( abs(x - μ) / α )^p )
end
