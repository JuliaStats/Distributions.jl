...
var(d::PGeneralizedGaussian) = (d.α^2) * (SFunc.gamma(3 * inv(d.p)) / SFunc.gamma(inv(d.p)))
std(d::PGeneralizedGaussian) = (d.α) * sqrt(SFunc.gamma(3 * inv(d.p)) / SFunc.gamma(inv(d.p)))

skewness(d::PGeneralizedGaussian{T1, T2, T3}) where {T1,T2,T3} = zero(T1)
kurtosis(d::PGeneralizedGaussian) = SFunc.gamma(5 * inv(d.p)) * SFunc.gamma(inv(d.p)) / (SFunc.gamma(3 * inv(d.p))^2) - 3
entropy(d::PGeneralizedGaussian) = inv(d.p) - log( d.p / (2 * d.α * SFunc.gamma(inv(d.p))))
...
function pdf(d::PGeneralizedGaussian, x::Real)
    (μ, α, p) = params(d)
    return ( p / ( 2α * SFunc.gamma(1 / p) ) ) * exp( -( abs(x - μ) / α )^p )
end
