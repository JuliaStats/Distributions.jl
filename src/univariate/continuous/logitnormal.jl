...
function LogitNormal(μ::T, σ::T; check_arg=true) where {T <: Real}
...
LogitNormal() = LogitNormal(0.0, 1.0, check_arg = false)
...
convert(::Type{LogitNormal{T}}, d::LogitNormal{S}) where
  {T <: Real, S <: Real} = LogitNormal(T(d.μ), T(d.σ), check_arg = false)
