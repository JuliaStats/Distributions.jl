...
function GeneralizedPareto(μ::T, σ::T, ξ::T; arg_check = true) where {T <: Real}
    arg_check && @check_args(GeneralizedPareto, σ > zero(σ))
...
GeneralizedPareto() = GeneralizedPareto(0.0, 1.0, 1.0, arg_check = false)
...
function convert(::Type{GeneralizedPareto{T}}, d::GeneralizedPareto{S}) where {T <: Real, S <: Real}
    GeneralizedPareto(T(d.μ), T(d.σ), T(d.ξ), arg_check = false)
end
