...
struct InverseGamma{T <: Real} <: ContinuousUnivariateDistribution
    invd::Gamma{T}
    θ::T
    InverseGamma{T}(α::T, θ::T) where {T <: Real} =
        new{T}(Gamma(α, inv(θ), check_args = false), θ)
end
...
InverseGamma(α::Real, θ::Real; check_args::Bool = true) =
    InverseGamma(promote(α, θ)...; check_args = check_args)
InverseGamma(α::Integer, θ::Integer; check_args::Bool = true) =
    InverseGamma(float(α), float(θ); check_args = check_args)
InverseGamma(α::Real; check_args::Bool = true) =
    InverseGamma(α, one(α); check_args = check_args)
InverseGamma() = InverseGamma{Float64}(1.0, 1.0)

@distr_support InverseGamma 0.0 Inf

#### Conversions
convert(::Type{InverseGamma{T}}, α::S, θ::S) where {T <: Real, S <: Real} =
    InverseGamma(T(α), T(θ))
...
function logpdf(d::InverseGamma, x::Real)
    (α, θ) = params(d)
    return α * log(θ) - loggamma(α) - (α + 1) * log(x) - θ / x
end
...
function mgf(d::InverseGamma{T}, t::Real) where {T <: Real}
    (a, b) = params(d)
    return t == zero(t) ? one(T) : 2(-b * t)^(0.5a) / gamma(a) * besselk(a, sqrt(-4 * b * t))
end

function cf(d::InverseGamma{T}, t::Real) where {T <: Real}
    (a, b) = params(d)
    return t == zero(t) ? one(T) + zero(T) * im :
        2(-im * b * t)^(0.5a) / gamma(a) * besselk(a, sqrt(-4 * im * b * t))
end
