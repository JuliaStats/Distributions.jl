...
SkewNormal(ξ::Real, ω::Real, α::Real; check_args::Bool = true) =
    SkewNormal(promote(ξ, ω, α)...; check_args = check_args)
SkewNormal(ξ::Integer, ω::Integer, α::Integer; check_args::Bool = true) =
    SkewNormal(float(ξ), float(ω), float(α); check_args = check_args)
...
#### Evaluation
pdf(d::SkewNormal, x::Real) =
    (2 / d.ω) * normpdf((x - d.ξ) / d.ω) * normcdf(d.α * (x - d.ξ) / d.ω)
...
cf(d::SkewNormal, t::Real) =
    exp(im * t * d.ξ - (d.ω^2 * t^2) / 2) *
    (1 + im * erfi((d.ω * delta(d) * t) / (sqrt(2))))
