...
function logpdf(d::Geometric, x::Real)
    return insupport(d, x) ? log(d.p) + log1p(-d.p) * x : log(zero(d.p))
end

function cdf(d::Geometric, x::Int)
    p = succprob(d)
    n = max(x + 1, 0)
    return p < 1 / 2 ? -expm1(log1p(-p) * n) : 1 - (1 - p)^n
end
...
function ccdf(d::Geometric, x::Int)
    p = succprob(d)
    n = max(x + 1, 0)
    return p < 1 / 2 ? exp(log1p(-p) * n) : (1 - p)^n
end
