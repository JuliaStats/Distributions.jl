...
Bernoulli(p::Integer; check_args::Bool = true) =
    Bernoulli(float(p); check_args = check_args)
...
function entropy(d::Bernoulli)
    p0 = failprob(d)
    p1 = succprob(d)
    return (p0 == 0 || p0 == 1) ? zero(d.p) : -(p0 * log(p0) + p1 * log(p1))
end

#### Evaluation

pdf(d::Bernoulli, x::Bool) = x ? succprob(d) : failprob(d)
pdf(d::Bernoulli, x::Real) = x == 0 ? failprob(d) : x == 1 ? succprob(d) : zero(d.p)

logpdf(d::Bernoulli, x::Real) = log(pdf(d, x))

cdf(d::Bernoulli, x::Bool) = x ? one(d.p) : failprob(d)
cdf(d::Bernoulli, x::Int) = x < 0 ? zero(d.p) : x < 1 ? failprob(d) : one(d.p)

ccdf(d::Bernoulli, x::Bool) = x ? zero(d.p) : succprob(d)
ccdf(d::Bernoulli, x::Int) = x < 0 ? one(d.p) : x < 1 ? succprob(d) : zero(d.p)
...
function cgf(d::Bernoulli, t)
    p, = params(d)
    # log(1-p+p*exp(t))
    return logaddexp(log1p(-p), t + log(p))
end
...
fit_mle(::Type{T}, ss::BernoulliStats) where {T <: Bernoulli} =
    T(ss.cnt1 / (ss.cnt0 + ss.cnt1))
