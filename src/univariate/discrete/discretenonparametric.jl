...
struct DiscreteNonParametric{T<:Real,P<:Real,Ts<:AbstractVector{T},Ps<:AbstractVector{P}} <: DiscreteUnivariateDistribution
    ...
    function DiscreteNonParametric{T,P,Ts,Ps}(vs::Ts, ps::Ps; check_arg=true) where {
            T<:Real,P<:Real,Ts<:AbstractVector{T},Ps<:AbstractVector{P}}
        check_arg || return new{T,P,Ts,Ps}(vs, ps)
...
DiscreteNonParametric(vs::Ts, ps::Ps; check_arg=true) where {
        T<:Real,P<:Real,Ts<:AbstractVector{T},Ps<:AbstractVector{P}} =
    DiscreteNonParametric{T,P,Ts,Ps}(vs, ps, check_arg = check_arg)
...
convert(::Type{DiscreteNonParametric{T,P,Ts,Ps}}, d::DiscreteNonParametric) where {T,P,Ts,Ps} =
    DiscreteNonParametric{T,P,Ts,Ps}(Ts(support(d)), Ps(probs(d)), check_arg = false)
...
fit_mle(::Type{<:DiscreteNonParametric},
    ...
    DiscreteNonParametric{T,W,Ts,Ws}(ss.support, pnormalize!(copy(ss.freq)), check_arg = false)
