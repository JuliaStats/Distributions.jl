...
sampler(d::DiscreteNonParametric) = DiscreteNonParametricSampler(support(d), probs(d))
...
insupport(d::DiscreteNonParametric, x::Real) = length(searchsorted(support(d), x)) > 0
