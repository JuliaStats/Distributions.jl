...
function Categorical(k::Integer; check_args=true)
    ...
    return Categorical{AbstractFloat,Vector}(Base.OneTo(k), fill(1/k, k), check_args=check_args)
end
...
function add_categorical_counts!(h::Vector, x::AbstractArray{T}) where T<:Integer
    for i = 1 : length(x)
        @inbounds xi = x[i]
...
function add_categorical_counts!(h::Vector, x::AbstractArray{T}, w::AbstractArray) where T<:Integer
    n = length(x)
    if n != length(w)
...
function suffstats(::Type{<:Categorical}, k::Int, x::AbstractArray{T}, w::AbstractArray) where T<:Integer
...
suffstats(::Type{<:Categorical}, data::CategoricalData, w::AbstractArray) = suffstats(Categorical, data..., w)
...
function fit_mle(::Type{<:Categorical}, k::Integer, x::AbstractArray{T}, w::AbstractArray) where T<:Integer
...
fit_mle(::Type{<:Categorical}, data::CategoricalData, w::AbstractArray) = fit_mle(Categorical, data..., w)

fit_mle(::Type{<:Categorical}, x::AbstractArray{T}) where {T<:Integer} = fit_mle(Categorical, maximum(x), x)
fit_mle(::Type{<:Categorical}, x::AbstractArray{T}, w::AbstractArray) where {T<:Integer} = fit_mle(Categorical, maximum(x), x, w)

fit(::Type{<:Categorical}, data::CategoricalData) = fit_mle(Categorical, data)
fit(::Type{<:Categorical}, data::CategoricalData, w::AbstractArray) = fit_mle(Categorical, data, w)
