...
logpdf(d::FisherNoncentralHypergeometric, k::Int) =
    -log(d.ns + 1) - SFunc.logbeta(d.ns - k + 1, k + 1) -
    log(d.nf + 1) - SFunc.logbeta(d.nf - d.n + k + 1, d.n - k + 1) +
    xlogy(k, d.ω) - _P(d, 0)
...
function logpdf(d::WalleniusNoncentralHypergeometric, k::Int)
    ...
    I, _ = quadgk(f, 0, 1)
    return -log(d.ns + 1) - SFunc.logbeta(d.ns - k + 1, k + 1) -
    log(d.nf + 1) - SFunc.logbeta(d.nf - d.n + k + 1, d.n - k + 1) + log(I)
end
