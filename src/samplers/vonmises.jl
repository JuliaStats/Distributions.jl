...
struct VonMisesSampler{T<:Real} <: Sampleable{Univariate,ContinuousSupport{T}}
    μ::T
    κ::T
    r::T

    function VonMisesSampler{T}(μ::T, κ::T) where {T}
...
function rand(rng::AbstractRNG, s::VonMisesSampler{T}) where {T}
    f = 0.0
    local x::T
    if s.κ > 700.0
