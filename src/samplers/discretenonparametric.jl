...
"""
struct DiscreteNonParametricSampler{T <: Real, S <: AbstractVector{T}, A <: AliasTable} <:
    Sampleable{Univariate, Discrete}
    support::S
    aliastable::A

    function DiscreteNonParametricSampler{T, S}(
            support::S,
            probs::AbstractVector{<:Real},
        ) where {T <: Real, S <: AbstractVector{T}}
        aliastable = AliasTable(probs)
        return new{T, S, typeof(aliastable)}(support, aliastable)
    end
end
...
DiscreteNonParametricSampler(
    support::S,
    probs::AbstractVector{<:Real},
) where {T <: Real, S <: AbstractVector{T}} = DiscreteNonParametricSampler{T, S}(support, probs)
