...
function quantile(d::EdgeworthZ, p::Float64)
    ...
    z2 = z * z
    return z + s * (z2 - 1) / 6.0 + k * z * (z2 - 3) / 24.0 - s * s / 36.0 * z * (2.0 * z2 - 5.0)
end
...
function pdf(d::EdgeworthAbstract, x::Float64)
    m, s = mean(d), std(d)
    return pdf(EdgeworthZ(d.dist, d.n), (x - m) / s) / s
end

cdf(d::EdgeworthAbstract, x::Float64) = cdf(EdgeworthZ(d.dist, d.n), (x - mean(d)) / std(d))

ccdf(d::EdgeworthAbstract, x::Float64) =
    ccdf(EdgeworthZ(d.dist, d.n), (x - mean(d)) / std(d))

quantile(d::EdgeworthAbstract, p::Float64) =
    mean(d) + std(d) * quantile(EdgeworthZ(d.dist, d.n), p)
cquantile(d::EdgeworthAbstract, p::Float64) =
    mean(d) + std(d) * cquantile(EdgeworthZ(d.dist, d.n), p)
