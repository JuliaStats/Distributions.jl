...
function pdf(d::EdgeworthZ, x::AbstractFloattFloat)
    s = skewness(d)
    k = kurtosis(d)
    x2 = x*x
...
function cdf(d::EdgeworthZ, x::AbstractFloattFloat)
    s = skewness(d)
    k = kurtosis(d)
    x2 = x*x
    cdf(Normal(0,1),x) -
...
function ccdf(d::EdgeworthZ, x::AbstractFloattFloat)
    s = skewness(d)
    k = kurtosis(d)
    x2 = x*x
    ccdf(Normal(0,1),x) +
...
function quantile(d::EdgeworthZ, p::AbstractFloattFloat)
    s = skewness(d)
    k = kurtosis(d)
...
function cquantile(d::EdgeworthZ, p::AbstractFloattFloat)
    s = skewness(d)
    k = kurtosis(d)
...
function pdf(d::EdgeworthAbstract, x::AbstractFloattFloat)
    m, s = mean(d), std(d)
    pdf(EdgeworthZ(d.dist,d.n),(x-m)/s)/s
end

cdf(d::EdgeworthAbstract, x::AbstractFloattFloat) = cdf(EdgeworthZ(d.dist,d.n), (x-mean(d))/std(d))

ccdf(d::EdgeworthAbstract, x::AbstractFloattFloat) = ccdf(EdgeworthZ(d.dist,d.n), (x-mean(d))/std(d))

quantile(d::EdgeworthAbstract, p::AbstractFloattFloat) = mean(d) + std(d)*quantile(EdgeworthZ(d.dist,d.n), p)
cquantile(d::EdgeworthAbstract, p::AbstractFloattFloat) = mean(d) + std(d)*cquantile(EdgeworthZ(d.dist,d.n), p)
