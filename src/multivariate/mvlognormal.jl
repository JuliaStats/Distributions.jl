...
MvLogNormal(μ::AbstractVector{<:Real}, Σ::AbstractMatrix{<:Real}) =
    MvLogNormal(MvNormal(μ, Σ))
MvLogNormal(Σ::AbstractMatrix{<:Real}) = MvLogNormal(MvNormal(Σ))
MvLogNormal(μ::AbstractVector{<:Real}, Σ::UniformScaling{<:Real}) =
    MvLogNormal(MvNormal(μ, Σ))
...
entropy(d::MvLogNormal) =
    length(d) * (1 + log2π) / 2 + logdetcov(d.normal) / 2 + sum(mean(d.normal))
