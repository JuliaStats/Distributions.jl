...
const ZeroMeanFullNormalCanon{Axes} = MvNormalCanon{PDMat,    Zers{Float64,1}}
const ZeroMeanDiagNormalCanon{Axes} = MvNormalCanon{PDiagMat, Zers{Float64,1}}
const ZeroMeanIsoNormalCanon{Axes}  = MvNormalCanon{ScalMat,  Zers{Float64,1,Axes}}
```
...
const ZeroMeanFullNormalCanon{Axes} = MvNormalCanon{Float64,PDMat{Float64,Matrix{Float64}},Zers{Float64,1,Axes}}
const ZeroMeanDiagNormalCanon{Axes} = MvNormalCanon{Float64,PDiagMat{Float64,Vector{Float64}},Zers{Float64,1,Axes}}
const ZeroMeanIsoNormalCanon{Axes}  = MvNormalCanon{Float64,ScalMat{Float64},Zers{Float64,1,Axes}}
...
# meanform{C, T<:Real}(d::MvNormalCanon{T,C,Zers{T}}) = MvNormal(inv(d.J))

function canonform(d::MvNormal{T,C,<:AbstractVector{T}}) where {C, T<:Real}
    J = inv(d.Σ)
    return MvNormalCanon(d.μ, J * collect(d.μ), J)
end
canonform(d::MvNormal{T,C,Zers{T}}) where {C, T<:Real} = MvNormalCanon(inv(d.Σ))
