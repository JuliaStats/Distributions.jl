...
"""
MvNormalCanon(h::AbstractVector{<:Real}, J::AbstractMatrix{<:Real}) =
    MvNormalCanon(h, PDMat(J))
MvNormalCanon(h::AbstractVector{<:Real}, J::Diagonal{<:Real}) =
...
Base.@deprecate MvNormalCanon(h::AbstractVector{<:Real}, prec::AbstractVector{<:Real}) MvNormalCanon(
    h,
    LinearAlgebra.Diagonal(prec),
)
Base.@deprecate MvNormalCanon(h::AbstractVector{<:Real}, prec::Real) MvNormalCanon(
    h,
    prec * I,
)
Base.@deprecate MvNormalCanon(prec::AbstractVector) MvNormalCanon(
    LinearAlgebra.Diagonal(prec),
)
Base.@deprecate MvNormalCanon(d::Int, prec::Real) MvNormalCanon(
    LinearAlgebra.Diagonal(FillArrays.Fill(prec, d)),
)
...
sqmahal!(r::AbstractVector, d::MvNormalCanon, x::AbstractMatrix) =
    quad!(r, d.J, broadcast(-, x, d.μ))
