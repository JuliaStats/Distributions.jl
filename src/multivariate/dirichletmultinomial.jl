...
"""
struct DirichletMultinomial{T<:Real,V<:AbstractVector{T}} <:
...
DirichletMultinomial(n::Integer, α::AbstractVector{T}) where {T<:Real} =
    DirichletMultinomial{T}(n, α)
DirichletMultinomial(n::Integer, α::AbstractVector{T}) where {T<:Integer} =
