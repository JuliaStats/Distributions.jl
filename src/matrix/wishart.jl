...
function Wishart(df::T, S::AbstractPDMat{T}) where {T <: Real}
    ...
    return Wishart{typeof(_df), typeof(S), typeof(rnk)}(_df, S, _logc0, rnk, singular)
end

function Wishart(df::Real, S::AbstractPDMat)
    T = Base.promote_eltype(df, S)
    return Wishart(T(df), convert(AbstractArray{T}, S))
end
...
function convert(::Type{Wishart{T}}, d::Wishart) where {T <: Real}
    ...
    return Wishart{T, typeof(P), typeof(d.rank)}(T(d.df), P, T(d.logc0), d.rank, d.singular)
end
...
function convert(
    ...
    return Wishart{T, typeof(P), typeof(rnk)}(T(df), P, T(logc0), rnk, singular)
end
