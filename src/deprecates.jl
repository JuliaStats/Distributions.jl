...
for fun in [
    ...
    @eval begin
        ...
        ) r .= Base.Fix1($fun, d).(X) false
        @deprecate ($fun!)(
            r::AbstractArray{<:Real},
            d::UnivariateDistribution,
            X::AbstractArray{<:Real},
        ) r .= Base.Fix1($fun, d).(X) false
        @deprecate ($fun)(d::UnivariateDistribution, X::AbstractArray{<:Real}) map(
            Base.Fix1($fun, d),
            X,
        )
    end
...
@deprecate expectation(distr::DiscreteUnivariateDistribution, g::Function, epsilon::Real) expectation(
    g,
    distr;
    epsilon = epsilon,
) false
@deprecate expectation(distr::ContinuousUnivariateDistribution, g::Function, epsilon::Real) expectation(
    g,
    distr,
) false
